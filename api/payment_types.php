...
function handleGet($conn, $table) {
    ...
    if ($id) {
        ...
        // Get all records
        $sql = "SELECT * FROM $table ORDER BY id ASC"; // Order by ID or name
        ...
        if ($result) {
            ...
            http_response_code(500);
            echo json_encode(["message" => "Error retrieving payment types: " . $conn->error]);
        }
    }
}
...
function handlePost($conn, $table) {
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode(["message" => "Payment type created successfully.", "id" => $newId]);
    } else {
        if ($conn->errno == 1062) { // Duplicate entry for name (UNIQUE constraint)
             http_response_code(409);
             echo json_encode(["message" => "Duplicate entry for payment type name."]);
        } else {
             http_response_code(500);
             echo json_encode(["message" => "Error creating payment type: " . $stmt->error]);
        }
    }
    $stmt->close();
}
    ...
    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            ...
            if ($checkResult->num_rows === 0) {
                 http_response_code(404);
                 echo json_encode(["message" => "Payment type not found for update."]);
            } else {
                 http_response_code(200);
                 echo json_encode(["message" => "Payment type found, but no changes were made."]);
            }
            $checkStmt->close();
        }
    } else {
         if ($conn->errno == 1062) {
             http_response_code(409);
             echo json_encode(["message" => "Update failed due to duplicate entry for payment type name."]);
         } else {
            http_response_code(500);
            echo json_encode(["message" => "Error updating payment type: " . $stmt->error]);
         }
    }
    $stmt->close();
}
...
function handleDelete($conn, $table) {
    ...
    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            ...
            http_response_code(404);
            echo json_encode(["message" => "Payment type not found for delete."]);
        }
    } else {
         if ($conn->errno == 1451) { // Foreign key constraint fails
             http_response_code(409);
             echo json_encode(["message" => "Cannot delete payment type because it is referenced by other data (e.g., in cargos or payment_management)."]);
        } else {
            http_response_code(500);
            echo json_encode(["message" => "Error deleting payment type: " . $stmt->error]);
        }
    }
    $stmt->close();
}
